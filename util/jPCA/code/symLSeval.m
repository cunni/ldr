...
% The best way to derive this is to consider the expanded vector form Hk,
% where k is a n(n+1)/2 'symmetric matrix' vector and H maps that to the 
% space of real matrices n^2.  Then the derivative has H^T, which maps the
% diagonal elements *singly* back to the derivative (no double counting),
% but otherwise takes the sum of the non-diagonal elements (D + D' term).
