...
% 2013
%
% minimize_grassmann_mosd.m
%
% This code performs minimization of a function f
% over the Grassmann manifold.
%
...
% 
% Outputs:
% - Q: the optimal Grassmann point for f
% - fQ: f evaluated at Q
%
...
function [ Q , fQ , info ] = minimize_grassmann_mosd( f , Q , length , varargin )
    ...
    [Q, fQ , info, options] = steepestdescent(problem , Q , options );
    
    fprintf('(Grassmann manifold manopt steepest descent opt completed on %s in %d iterations, objective optimized from %g to %g)\n',f,info(end).iter, fQ_0, fQ);
    %
end    